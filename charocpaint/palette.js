...
//--------------------------------------------------------------
	fillPalette2() {  //contains both lightness and saturation cycle 
		for (var i = 0; i < this.rows*0.5; i++) {
			this.colorCycler.reset();
			
			this.colorCycler.setSaturation(((i+1)/(this.rows*0.5))*90  + 5);
		...
		for (var i = this.rows*0.5; i < this.rows; i++) {
			this.colorCycler.reset();
			
			let val = this.normalize(i, this.rows*0.5, this.rows) * 100;
			val = val*0.7+25;
			this.colorCycler.setLightness(100-val);
