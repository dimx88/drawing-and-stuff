...
var global = {selected_color:null, image_smoothing:true};
...
function sampleColor(x, y) {
	   let _data = ctx.getImageData(x, y, 1, 1).data;
	   let r = _data[0];
	   let g = _data[1];
	   let b = _data[2];
...
this.onmousemove = function(e) {
	...
	if (mouse.right_down && !mouse.left_down && imageCropper.path.length > 3) {
		stamp();
	}
...
this.onmousedown = function(e) {
	...
	brush_alpha = easeInSine(slider.value);
	if (e.target !== canvas) return;
	...
	if (e.button === 2 && imageCropper.path.length > 3) {
	...
	if (e.button === 1) {
		global.selected_color = sampleColor(mouse.x, mouse.y);
		console.log('color changed ' + global.selected_color);
		slider.render();
	}
